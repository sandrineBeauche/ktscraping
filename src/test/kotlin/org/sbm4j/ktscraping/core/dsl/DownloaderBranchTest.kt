...
class DownloaderBranchTest: CrawlerTest() {
    ...
    suspend fun sendStartEvent(){
        ...
        val back = crawlerChannelManager.downloaderChannel.sendSync<EventBack>(startEvent)
    }

    suspend fun sendEndEvent(){
        val endEvent = EndEvent(sender)
        val back = crawlerChannelManager.downloaderChannel.sendSync<EventBack>(endEvent)
    }
    ...
    fun testBuildCrawlerWithDownloaderDispatcher() = scope.runTest{
        ...
        val c = crawler("MainCrawler", ::testDIModule) {
            downloaderDispatcher(
                "dispatcher1",
                { req: AbstractRequest ->
                    if (req is DownloadingRequest && req.url == url1) channelOuts[0]
                    else channelOuts[1]
                })
            {
                downloader<DownloaderClassTest>(name = "Downloader1")
        ...
        crawlerChannelManager.downloaderChannel.send(request1)
        crawlerChannelManager.downloaderChannel.send(request2)

        response1 = crawlerChannelManager.downloaderChannel.receiveBack<DownloadingResponse>()
        response2 = crawlerChannelManager.downloaderChannel.receiveBack<DownloadingResponse>()
    ...
    fun testBuildCrawlerWithDownloaderDispatcherAndBranch() = scope.runTest{
        ...
        val response1: DownloadingResponse = crawlerChannelManager.downloaderChannel.receiveBack<DownloadingResponse>()
        val response2: DownloadingResponse = crawlerChannelManager.downloaderChannel.receiveBack<DownloadingResponse>()
